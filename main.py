...
experimentdict = {1.3: "30", 1.6: "60", 1.9: "90", 1.12: "120",
                  ...
                  2.4: "40", 3: "Glasses Worn", 4.1: "Left", 4.2: "Right",
...
def generategroundtruths():  # generates truths, which is an array of ground truth pairs to be evaluated against
    iterate = [0, 1, 2, 3, 4]  # used to iterate in for loops
    ...
    for d in distances:
        ...
        for i in iterate:  # calculates the xgaze and ygaze of the user for each of the points A-E
            xgaze = math.atan((eye_location[0] - original[i][0]) / eye_location[2])
            ygaze = math.atan((eye_location[1] - original[i][1]) / eye_location[2])
            temptruths.append([xgaze, ygaze])
        truths.append(temptruths)

    return truths  # 0-4: five points for five distances


def generatemovinggroundtruths():  # appends the groundtruths for the moving dot experiments.
    original = [5.52, 15]
    eye_location = [26.5, 15, 60]
    x_inc = [4.5, 6.75, 13.5]
    steps = [340, 222, 103]
    ...
    for i in range(0, 3):
        ...
        for s in range(0, steps[i]):
            delta = x_inc[i] * s * 53 / 1920
            x_p = original[0] + delta
            xgaze = math.atan((eye_location[0] - x_p) / eye_location[2])
            ygaze = 0
            temptruths.append([xgaze, ygaze])
        truths.append(temptruths)  # 5-7: three arrays for slow, medium, and fast
...
def calculateerror(currentfile, experimentnumber):
    grouperror = []
    abscsverror = []
    relcsverror = []
    currenterror = []

    if experimentnumber == 5.6 or experimentnumber == 5.8 or experimentnumber == 5.12:
...
def mov_calculateerror(currentfile, experimentnumber):
    releventtruths = []
    abscsverror = []
    relcsverror = []
    print(currentfile)
    ...
    if experimentnumber == 5.12:  # slow
        releventtruths = truths[5]
    elif experimentnumber == 5.8:  # medium
        releventtruths = truths[6]
    elif experimentnumber == 5.6:  # fast
        releventtruths = truths[7]

    currentdata = currentdata.reset_index()
    for index, row in currentdata.iterrows():
        if index == len(releventtruths):
            break
        relxerror = (abs(releventtruths[index][0] - row['x_1'])) / releventtruths[0][0]
        relyerror = (abs(releventtruths[index][1] - row['x_2']))  # / releventtruths[0][1]
        absxerror = (abs(releventtruths[index][0] - row['x_1']))
        absyerror = (abs(releventtruths[index][1] - row['x_2']))
        graphingdatax.append(row['x_1'])
        graphingdatay.append(row['x_2'])
        abscsverror.append([absxerror, absyerror])
        relcsverror.append([relxerror, relyerror])
    ...
    if experimentnumber == 5.6:
        ...
        for filename in os.scandir("/home/kre8or/PycharmProjects/analysis/experimentdata"):
            if str(filename).__contains__("fast-movement") and str(filename).__contains__(".csv"):
                experimentcounter += 1
                if mov_calculateerror(filename.name, experimentnumber) == -1:
                    experimentcounter -= 1
        printcsvcounter(experimentcounter)
        ...
        for filename in os.scandir("/home/kre8or/PycharmProjects/analysis/experimentdata"):
            if str(filename).__contains__("IR-projector") and str(filename).__contains__(".csv"):
                ...
                if calculateerror(filename.name, experimentnumber) == -1:
                    experimentcounter -= 1
        printcsvcounter(experimentcounter)
    if experimentnumber == 9.1:
        experimentnumber = 9.1
        experimentcounter = 0
        for filename in os.scandir("/home/kre8or/PycharmProjects/analysis/experimentdata"):
            if str(filename).__contains__("low-lighting") and str(filename).__contains__(".csv"):
                experimentcounter += 1
                if calculateerror(filename.name, experimentnumber) == -1:
                    experimentcounter -= 1
        printcsvcounter(experimentcounter)
    if experimentnumber == 9.2:
        experimentnumber = 9.2
        experimentcounter = 0
        for filename in os.scandir("/home/kre8or/PycharmProjects/analysis/experimentdata"):
